...
interface Istate{
    userInfo:IUserInfo
    state:'loading'|'rejected'|'fullfiled'| 'initial',
    Msg:any
}
...
export const signUpUser = createAsyncThunk<IUserInfo, Partial<IUser>>('User/register', async (value, {rejectWithValue}) => {
      try {
        ...
        localStorage.setItem('userToken', json.data._id)
...
//     try{
//         const json = await axios.get('http://localhost:3002/login')
//         return json.data
//     }catch(error){
//         console.log(error)
//     }
